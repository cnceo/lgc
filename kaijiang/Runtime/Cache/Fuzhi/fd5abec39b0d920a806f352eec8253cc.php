<?php if (!defined('THINK_PATH')) exit();?>﻿<!DOCTYPE html>
<html>

	<head>

		<meta charset="utf-8" />
		<meta name="format-detection" content="telephone=no" />
		<meta http-equiv="X-UA-Compatible" content="IE=EmulateIE11" />
		<title>看彩票开奖结果直播，彩票走势图，稳赚技巧方法,来168彩票开奖网，官方数据!精准无延迟！ </title>
		<meta name="keywords" content="168彩票开奖结果,168彩票开奖直播,168彩票走势图,168彩票稳赚技巧,168彩票赚钱方法" />
		<meta name="description " content="168彩票开奖网是彩种最全、开奖最快的专业彩票开奖网站。北京PK10、时时彩、极速赛车、双色球、大乐透…一应俱全！" />
		<link rel="stylesheet" href="/css/css/headorfood.css" />
		<link rel="shortcut icon" href="/images/images/icon/168favicon.ico?v=201889947">
		<link rel="stylesheet" href="/css/css/index.css" />
		<link rel="shortcut icon" href="/images/images/icon/168favicon.ico?v=201889947">
		<link rel="stylesheet" href="/css/css/user_adv.css" />
		<link rel="stylesheet" href="/css/css/idangerous.swiper.css" />
		<script src="/js/js/jquery.min.js"></script>
		<script src="/js/js/bootstrap.min.js"></script>
		<script type="text/javascript" src="/js/js/jquery-1.7.2.min.js"></script>
		<script type="text/javascript" src="/js/js/jquery.superslide.2.1.1.js"></script>
		<script type="text/javascript" src="/js/js/config.js"></script>
	</head>


	<script>
		tools.browserRedirect();
	</script>

	<body>
		<div class="bodybox">
			<div class="bgdiv">
				<script>
	//google统计代码
	(function(i, s, o, g, r, a, m) {
		i['GoogleAnalyticsObject'] = r;
		i[r] = i[r] || function() {
			(i[r].q = i[r].q || []).push(arguments)
		}, i[r].l = 1 * new Date();
		a = s.createElement(o),
			m = s.getElementsByTagName(o)[0];
		a.async = 1;
		a.src = g;
		m.parentNode.insertBefore(a, m)
	})(window, document, 'script', 'https://www.google-analytics.com/analytics.js', 'ga');
	ga('create', 'UA-89926923-1', 'auto');
	ga('send', 'pageview');
</script>
<!--
	时间：2016-12-12
	描述：头部公公部分
-->
<div class="headboxf">
	<div class="headboxh">
		<div class="headboxhc">
			<div class="headboxhr">
				<ul>
					<!--<li class="logonli liline">
						<a href="javascript:void(0)" class="login">登录</a>
						<a href="javascript:void(0)" class="register">注册</a>
					</li>
					<li class="logonli liline">|</li>-->
					<li class="logonli">
						<a href="javascript:void(0)" onclick="SetHome(window.location)">设为首页</a>
					</li>
					<li class="logonli liline">|</li>
					<li class="logonli">
						<a href="javascript:void(0)" onclick="javascript:addFavorite2()">收藏本站</a>
					</li>
					<li class="logonli liline">|</li>
					<li class="logonli">
						<a href="/view/wanfaguize/PK10_wanfaguize.html">帮助中心</a>
					</li>
					<li class="logonli liline">|</li>
					<li class="miniIphone">
						<a href="javascript:void()">手机版</a>
					</li>
				</ul>
			</div>
			<div class="headboxhl" id="hotbox">
				<div class="hoticon">
					最新公告：
				</div>
				<div class="hotcontent">
					<marquee direction="left" behavior="scroll" scrollamount="6" onmouseover=this.stop() onmouseout=this.start()>
						<!--<span class="hoticon"></span>--><span class="textspace" id="noticeContent"></span>
					</marquee>
				</div>
				<!--<ul>
					<li class="logonli">
						<a href="http://old.1680210.com/" target="_blank">回到旧版</a>
					</li>
					<li class="logonli liline"><img src="picture/back.png" alt="" /></li>
					<li class="logonli">
						<a href="">注册</a>
					</li>
				</ul>-->
			</div>
		</div>
	</div>
	<div class="headboxh2">
		<div class="headbox">
			<div>
				<a class="logobox" href="/"  style="width: 305px;margin-left: -5px">
					<img src="/style/images/logo.gif" height="90px">
				</a>
			</div>
			<div class="advertisebox" style="width: 700px">
				<img src="/style/images/yz.png">
			</div>
			<div class="loginbox">
				<ul>
					<!--<li class="kefuicon">
					</li>-->
					<li class="logonli jianyue">
						<!--<a href="/view/public/jianyue.html?v=201889947">简约版</a>-->
						<div class="new_mobiledetail">
							<img src="/style/images/m.jpg" alt="" />
							<p class="sweep">扫一扫，下载移动客户端</p>
							<p class="remind">（IOS、Android均可扫码下载）</p>
						</div>
					</li>
				</ul>
			</div>
		</div>
	</div>
	<!--<div class="backoldbox">
		<a href="http://old.1680210.com/" target="_blank"></a>
	</div>-->
</div>
<div class="menubannbox heightauto orangeBG">
	<div class="menubox">
		<div class="menuboxr">
			<ul>
				<li class="li_checked nav_li">
					<a href="/">首页</a>
				</li>
				<li class="cphost nav_li">
					<a href="javascript:void(0)">彩票大厅
						<img class="cphost_pic1" src="/picture/cpdt_white.png" alt="" />
						<img class="cphost_pic2" src="/picture/cpdt_red.png" alt="" />
					</a>
					<div class="cphost_content">
						<div class="content_middle">
							<div class="RMC">
								<div class="LineOne">
									<span class="RMC_span"><span>热门彩</span></span>
									<ul class="RMC_List">
										<!--香港彩 北京PK10 重庆时时彩 极速赛车 极速时时彩 江苏快3 极速飞艇 新疆时时彩 广西快乐十分 天津时时彩 PC蛋蛋幸运28-->
										<li>
											<a target="_blank" href="/pk10">北京PK10</a>
										</li>
										<li>
											<a target="_blank" href="/cqssc">重庆时时彩</a>
										</li>
										<li>
											<a target="_blank" href="/jsssc">极速时时彩</a>
										</li>
										<li>
											<a target="_blank" href="/jsk3">江苏快3</a>
										</li>
										<li>
											<a target="_blank" href="/xyft">幸運飞艇</a>
										</li>
										<li>
											<a target="_blank" href="javascript:void(0)" id="pc28">加拿大28</a>
										</li>
										<li>
											<a target="_blank" href="javascript:void(0)" id="bjft">北京番攤</a>
										</li>
										<li>
											<a target="_blank" href="/tjssc">天津时时彩</a>
										</li>
										<li>
											<a target="_blank" href="javascript:void(0)" id="cqft">重慶番攤</a>
										</li>
									</ul>

								</div>
							</div>
							<div class="JISUC">
								<div class="LineOne">
									<span class="JISUC_span"><span>极速彩</span></span>
									<ul>
										<!--极速赛车 极速飞艇 极速时时彩 极速快3 极速快乐十分 极速快乐8 极速11选5-->
										<li>
											<a target="_blank" href="/jssc">极速赛车</a>
										</li>
										<li>
											<a target="_blank" href="/jsssc">极速时时彩</a>
										</li>
									</ul>
								</div>
							</div>
							<div class="GPC">
								<div class="LineOne">
									<span class="GPC_span"><span>高频彩</span></span>
									<div class="rul">
										<ul>
											<!--江苏快3 吉林快3 河北快3 安徽快3 内蒙古快3 福建快3 湖北快3 北京快3 广西快3 广东11选5 上海11选5 安徽11选5 江西11选5 吉林11选5 广西11选5 湖北11选5 辽宁11选5 江苏11选5 浙江11选5 内蒙古11选5 十一运夺金 重庆幸运农场 广东快乐十分 天津快乐十分 北京快乐8 重庆七星彩 腾讯分分彩-->
											<li>
												<a target="_blank" href="/gdkl10">广东快乐十分</a>
											</li>
											<li>
												<a target="_blank" href="/bjk8">北京快乐8</a>
											</li>
										</ul>
										<ul>
											<li>
												<a target="_blank" href="/jsk3">江苏快3</a>
											</li>
											</ul>
											<ul>
											<li>
												<a target="_blank" href="/gd11x5">广东11选5</a>
											</li>
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_shanghai/index.html">上海11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_anhui/index.html">安徽11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_jiangxi/index.html">江西11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_jiling/index.html">吉林11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_guangxi/index.html">广西11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_hubei/index.html">湖北11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_liaoning/index.html">辽宁11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_jiangsu/index.html">江苏11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_zhejiang/index.html">浙江11选5</a>-->
											<!--</li>-->
											<!--<li>-->
												<!--<a target="_blank" href="/view/shiyix5_neimenggu/index.html">内蒙古11选5</a>-->
											<!--</li>-->
										</ul>
									</div>
								</div>
							</div>
							<!--<div class="JWC">-->
								<!--<div class="LineOne">-->
									<!--<span class="JWC_span"><span>境外彩</span></span>-->
									<!--<ul>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/aozxy5/ssc_index.html">澳洲幸运5</a>-->
										<!--</li>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/aozxy8/klsf_index.html">澳洲幸运8</a>-->
										<!--</li>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/aozxy10/pk10kai.html">澳洲幸运10</a>-->
										<!--</li>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/aozxy20/aozxy20_index.html">澳洲幸运20</a>-->
										<!--</li>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/taiwanbg/twbg_index.html">台湾宾果</a>-->
										<!--</li>-->
									<!--</ul>-->
								<!--</div>-->
							<!--</div>-->
							<div class="QGC">
								<div class="LineOne">
									<span class="QGC_span"><span>全国彩</span></span>
									<ul>
										<!--<li>-->
											<!--<a target="_blank" href="/view/fcssq/index.html">双色球</a>-->
										<!--</li>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/fc3D/index.html">福彩3D</a>-->
										<!--</li>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/fc7lc/index.html">七乐彩</a>-->
										<!--</li>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/cjdlt/index.html">大乐透</a>-->
										<!--</li>-->
										<li>
											<a target="_blank" href="/pl3">排列3</a>
										</li>
										<!--<li>-->
											<!--<a target="_blank" href="/view/tcpl5/index.html">排列5</a>-->
										<!--</li>-->
										<!--<li>-->
											<!--<a target="_blank" href="/view/tc7xc/index.html">七星彩</a>-->
										<!--</li>-->
									</ul>
								</div>
							</div>
						</div>
					</div>
				</li>
				<li id="zhusaHao_li">
					<a href="" target="_blank" id="zhusaHao">
						站长推荐
						<img class="master_pic1" src="/picture/cpdt_white.png" alt="" />
						<img class="master_pic2" src="/picture/cpdt_red.png" alt="" />
					</a>
					<img src="/picture/news.png" alt="" id="zhusaHao_img" />

				</li>

				<li id="new_zhao">
					<a href="/view/zhuiShaHao/PK10ganto.html" target="_blank">两面推荐</a>
					<img src="picture/news.png" alt="" id="ZSHao_img" />
				</li>
				<li>
					<a href="/view/changltx/index.html" target="_blank">长龙提醒</a>
				</li>
				<li>
					<a class="kaijdy" href="javascript:void(0)" target="_blank" rel="nofollow" id="kjdy">开奖调用</a>
				</li>
				<li class="zstb_nav nav_li">
					<a href="/view/zoushitb/zoushitb.html">走势图表 </a>
				</li>
				<li id="tryplay">
					<a href="javascript:void(0)" target="_blank" class="cqft">试玩投注</a>
				</li>
				<li class="zixun_nav nav_li">
					<a href="javascript:void(0)" target="_blank" class="cqft">资讯</a>
				</li>
				<li class="zixun_nav nav_li">
					<a href="javascript:void(0)" target="_blank" class="cqft">软件下载</a>
				</li>

				<li class="lasli">
					<div class="lasli_div">
						<!--<a id="msit" href="javascript:toM()" target="_blank" class="phoneicon">手机端</a>-->
						<a id="msit" href="/view/public/jianyue.html?v=201889947" target="_self" class="phoneicon">简约版</a>
						<!--<div class="mobilebox mobiledetailnew">
							<div class="mobiledetail">
								<img src="picture/168phone.png" alt="" />
								<p class="sweep">扫一扫，下载移动客户端</p>
								<p class="remind">（IOS、Android均可扫码下载）</p>
							</div>
						</div>-->
					</div>
				</li>
			</ul>
			<script>
                $("#kjdy").click(function () {
                    alert("敬請期待！");
                    return false;
                });
                $("#cqft").click(function () {
                    alert("敬請期待！");
                    return false;
                });
                $(".cqft").click(function () {
                    alert("敬請期待！");
                    return false;
                });
                $("#pc28").click(function () {
                    alert("敬請期待！");
                    return false;
                });
                $("#bjft").click(function () {
                    alert("敬請期待！");
                    return false;
                });

			</script>
		</div>
	</div>
</div>
<script type="text/javascript" src="/js/js/tools.js"></script>
<script>
	$(function() {
		//		tools.testWWW(); //判断去广告
		tools.advertisement(); //加载广告
		tools.WWWD(); //天豪去掉试玩投注
		tools.noAPP();
		//手机端二维码
		//二维码
		$(".lasli_div>a,.mobilebox").on("mouseover", function(e) {
			$(".mobiledetail").stop().animate({
				height: "295px",
				opacity: "1",
				marginTop: "60px"
			}, 200);
			$(".mobilebox").css({
				"overflow": "inherit"
			});
		});
		$(".mobilebox").mouseout(function() {
			$(".mobiledetail").stop().animate({
				height: "0",
				opacity: "1",
				marginTop: "0"
			}, 200);
			$(".mobilebox").css({
				"overflow": "inherit"
			});
		});
		//彩票大厅
		$(".cphost").on("mouseover", function() {
			$(".cphost_pic1").hide();
			$(".cphost_pic2").show();
			$(".cphost_content").show();
		});
		$(".cphost").on("mouseout", function() {
		    $(".cphost_pic1").show();
		    $(".cphost_pic2").hide();
		    $(".cphost_content").hide();
		});
		//站长推荐
		$("#zhusaHao_li").on("mouseover", function() {
			$(".master_pic1").hide();
			$(".master_pic2").show();
			$(".zshContent").show();
		});
		$("#zhusaHao_li").on("mouseout", function() {
			$(".master_pic1").show();
			$(".master_pic2").hide();
			$(".zshContent").hide();
		});
		//彩种，鼠标移进来改变边框颜色,click 添加class
		$("#lotteryType>ul>li:not(.disabled)").on({
			mouseenter: function() {
				if($(this).hasClass("check")) {
					return
				}
				$(this).css("border-color", "#FA8E19");
			},
			mouseout: function() {
				if($(this).hasClass("check")) {
					return
				}
				$(this).css("border-color", "#E0E0E0");
			},
			click: function() {
				console.log(this)
				var showElem = $(this).attr("data-text");
				//判断是否在追杀号页面点击
				var Location = window.location.href.indexOf("/view/zsh/index");
				if(Location == -1) {
					window.open("/view/zsh/index.html?lottype=" + showElem, "_blank");
				}
				clearTimeout(timeOutInit);
				FunObj.isRan(showElem);
				$(this).addClass("check").css("border-color", "#FA8E19").siblings().removeClass("check");
				$(".lotteryType>ul>li:not(.check)").css("border-color", "#E0E0E0");
				lot_type = $(this).attr("data-text");
				FunObj.orzhuihao(lot_type);
				FunObj.introduction($(this).text());
			}
		})

		$(".menuboxr").find(".nav_li").on("click", function() {
			$(this).addClass("li_checked").siblings().removeClass("li_checked");
		})
		if($("#ifindex").val() == "index") {
			$(".mobiledetailnew").remove();
		};
		toolBoxs.setK(); //为开奖调用增加域名

	});

	function toM() {
		var mUrl = "//m." + tools.YM() + "" + tools.getCom();
		return mUrl;
	}
	//加载登录注册
	tools.loadloginR("login", "register");
</script>
				<div class="menubannbox menubannboxc">
					<div class="menubox">
						<div class="zhezao">
							<div class="zhezao_content">

								<div class="kair">
									<div class="announce_top">
										<style>
											.banner ul, ol { padding: 0;}
											.banner { position: relative; overflow: auto; text-align: center;overflow-y: hidden; overflow-x: hidden; 	}
											.banner li { list-style: none; }
											.banner ul li { float: left; }
											#b04 { width: 520px;}
											#b04 .dots { position: absolute; left: 0; right: 0; bottom: 20px;}
											#b04 .dots li
											{
												display: inline-block;
												width: 10px;
												height: 10px;
												margin: 0 4px;
												text-indent: -999em;
												border: 2px solid #fff;
												border-radius: 6px;
												cursor: pointer;
												opacity: .4;
												-webkit-transition: background .5s, opacity .5s;
												-moz-transition: background .5s, opacity .5s;
												transition: background .5s, opacity .5s;
											}
											#b04 .dots li.active
											{
												background: #fff;
												opacity: 1;
											}
											#b04 .arrow { position: absolute; top: 150px;}
											#b04 #al { left: 15px;}
											#b04 #ar { right: 15px;}
										</style>
										<div id="video_list" style="width: 523px">
											<div class="banner" id="b04">
												<ul>
													<li><img src="/images/cqssc-index.png.jpg" alt="" width="520" height="300" ></li>
													<li><img src="/images/gdkl10-index.png.jpg" alt="" width="520" height="300" ></li>
													<li><img src="/images/fc3d-index.png.jpg" alt="" width="520" height="300" ></li>
													<li><img src="/images/jxssc-index.png.jpg" alt="" width="520" height="300" ></li>
													<li><img src="/images/kl8-index.png.jpg" alt="" width="520" height="300" ></li>
												</ul>
												<a href="javascript:void(0);" class="unslider-arrow04 prev"><img class="arrow" id="al" src="/js/js/unslider/arrowl.png" alt="prev" width="20" height="35"></a>
												<a href="javascript:void(0);" class="unslider-arrow04 next"><img class="arrow" id="ar" src="/js/js/unslider/arrowr.png" alt="next" width="20" height="37"></a>
											</div>
										</div>
										<script type="text/javascript" src="/js/js/unslider/unslider.min.js"></script>
										<script>
											$(document).ready(function(e) {
												var unslider04 = $('#b04').unslider({
															dots: true
														}),
														data04 = unslider04.data('unslider');

												$('.unslider-arrow04').click(function() {
													var fn = this.className.split(' ')[1];
													data04[fn]();
												});
											});
										</script>
										<div class="announce_middle">
											<h1><span class="newAnn">行业新闻</span><span class="more"><a href="javascript:void(0)">更多</a></span></h1>
											<div class="margin_top" id="bannernews">
												<input type="hidden" id="bannernewsdata" value="[{&#34;releaseDate&#34;:&#34;2018-08-17&#34;,&#34;image&#34;:&#34;&#34;,&#34;title&#34;:&#34;“福彩连心”助孙女帮爷爷看病的梦&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:&#34;&#34;,&#34;newsId&#34;:2746,&#34;description&#34;:&#34;炎炎夏日的七月，一张来自中国药科大学的录取通知书，给王余俊一家带来了心灵上的丝丝凉意。今年，王余俊的孙女以365分的优异成绩被中国药科大学中药学专业录取，家人都很兴奋激动。可是，爷爷在却因为不能亲自送孙女入学而频频叹息，孙女也因为爷爷身体不&#34;},{&#34;releaseDate&#34;:&#34;2018-08-17&#34;,&#34;image&#34;:&#34;&#34;,&#34;title&#34;:&#34;莆田彩民喜获双色球百万元大奖  整晚算奖金&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:&#34;&#34;,&#34;newsId&#34;:2744,&#34;description&#34;:&#34;8月9日晚，双色球第2018092期开奖，福建彩民共拿下了8注单注奖金为26万多元的二等奖，位于莆田市荔城区镇海街道南西湖路的福彩35043156投注站中出其中4注，&#34;},{&#34;releaseDate&#34;:&#34;2018-08-17&#34;,&#34;image&#34;:&#34;&#34;,&#34;title&#34;:&#34;江苏快3推荐（08.17）：上午推荐&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:&#34;&#34;,&#34;newsId&#34;:2743,&#34;description&#34;:&#34;关注的号码：2.4.4；1.4.5；2.4.5；2.4.6(追号期数为：第4-29期)&#34;},{&#34;releaseDate&#34;:&#34;2018-08-17&#34;,&#34;image&#34;:&#34;&#34;,&#34;title&#34;:&#34;安徽快3推荐（08.17）：上午推荐&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:&#34;&#34;,&#34;newsId&#34;:2742,&#34;description&#34;:&#34;关注的号码：2.3.4；2.4.4；2.3.5；2.4.5；2.3.6；2.5.5；2.4.6；2.5.6(追号期数为：第4-29期)&#34;},{&#34;releaseDate&#34;:&#34;2018-08-17&#34;,&#34;image&#34;:&#34;&#34;,&#34;title&#34;:&#34;湖北快3推荐（08.17）：上午推荐&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:&#34;&#34;,&#34;newsId&#34;:2741,&#34;description&#34;:&#34;关注的号码：1.4.5；2.4.5(追号期数为：第4-29期)&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;&#34;,&#34;title&#34;:&#34;大乐透第18095后区半世琉璃形态组合分析：下期推荐&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:&#34;&#34;,&#34;newsId&#34;:2740,&#34;description&#34;:&#34;从最近十期的大小型态数据来看，两大号组合开奖次数为：1；两小号组合开奖次数为：2；大小各一组合开奖次数为：7。下期需重点关注两大号组合。&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;&#34;,&#34;title&#34;:&#34;重庆时时彩投注技巧：大小单双中奖实战技巧&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:&#34;&#34;,&#34;newsId&#34;:2739,&#34;description&#34;:&#34;时时彩其中两位数的只需猜对开奖号码的最后两位的单双，而于广大彩民而言，是颇有吸引力的，因为无论“大小&#34;还是“单双”，都是数字粗简形态的概括，&#34;}]">
												
												<p>
													<a target="_blank" href="javascript:void(0)">
														“福彩连心”助孙女帮爷爷看病的梦
													</a>
												</p>
												
												<p>
													<a target="_blank" href="javascript:void(0)">
														莆田彩民喜获双色球百万元大奖  整晚算奖金
													</a>
												</p>
												
												<p>
													<a target="_blank" href="javascript:void(0)">
														江苏快3推荐（08.17）：上午推荐
													</a>
												</p>
												
												<p>
													<a target="_blank" href="javascript:void(0)">
														安徽快3推荐（08.17）：上午推荐
													</a>
												</p>
												
												<p>
													<a target="_blank" href="javascript:void(0)">
														湖北快3推荐（08.17）：上午推荐
													</a>
												</p>
												
												<p>
													<a target="_blank" href="javascript:void(0)">
														大乐透第18095后区半世琉璃形态组合分析：下期推荐
													</a>
												</p>
												
												<p>
													<a target="_blank" href="javascript:void(0)">
														重庆时时彩投注技巧：大小单双中奖实战技巧
													</a>
												</p>
												
											</div>
										</div>
									</div>
									<div class="video_box">
										<span class="v_left"></span>
										<span class="v_right"></span>
										<ul>
											<li class="pk10">
												<a href="pk10">
													<dl>
														<dt>
													<img src="picture/pk10.png" alt="" />
													<p>
														<span></span>
														<span>北京PK10</span>
													</p>
												</dt>
														<dd class="dd_pak10">
															<!--倒计时：<span>07 </span> 分 <span> 50 </span> 秒-->
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>
											<li class="xingyft">
												<a href="xingyft">
													<dl>
														<dt>
													<img src="picture/jsft.png" alt="" />
													<p>
														<span></span>
														<span>极速飞艇</span>
													</p>
												</dt>
														<dd class="dd_pak10">
															<!--倒计时：<span>07 </span> 分 <span> 50 </span> 秒-->
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>
											<li class="gxklsf">
												<a href="gxklsf">
													<dl>
														<dt>
													<img src="picture/gxklsf.png" alt="" />
													<p>
														<span></span>
														<span>广西快乐十分</span>
													</p>
												</dt>
														<dd class="dd_pak10">
															<!--倒计时：<span>07 </span> 分 <span> 50 </span> 秒-->
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>
											<li class="fc3d">
												<a href="fcsd">
													<dl>
														<dt>
													<img src="picture/fc3d_v.jpg" alt="" />
													<p>
														<span></span>
														<span>福彩3D</span>
													</p>
												</dt>
														<dd class="dd_pak10">
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>
											<li class="bjk3">
												<a href="bjft">
													<dl>
														<dt>
													<img src="picture/bjks.png" alt="" />
													<p>
														<span></span>
														<span>北京快3</span>
													</p>
												</dt>
														<dd class="dd_pak10">
															<!--倒计时：<span>07 </span> 分 <span> 50 </span> 秒-->
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>
											<!--<li class="pcdd">
												<a href="egxy28">
													<dl>
														<dt>
															<img src="picture/pcdd.png" alt="" />
															<p>
																<span></span>
																<span>PC蛋蛋28</span>
															</p>
														</dt>
														<dd class="dd_pak10">
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>-->

											<li class="cqnc">
												<a href="cqxync">
													<dl>
														<dt>
													<img src="picture/cqnc_v.jpg" alt="" />
													<p>
														<span></span>
														<span>重庆幸运农场</span>
													</p>
												</dt>
														<dd class="dd_pak10">
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>
											<li class="shiyix5_sd">
												<a href="sdsyydj">
													<dl>
														<dt>
													<img src="picture/syydj_v.png" alt="" />
													<p>
														<span></span>
														<span>十一运夺金</span>
													</p>
												</dt>
														<dd class="dd_pak10">
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>
											<li class="tjklsf">
												<a href="tjklsf">
													<dl>
														<dt>
													<img src="picture/tjklsf_v.jpg" alt="" />
													<p>
														<span></span>
														<span>天津快乐十分</span>
													</p>
												</dt>
														<dd class="dd_pak10">
															<div class="cuttime">
																倒计时:
																<span class="bgtime hour">&nbsp;&nbsp;</span>
																<span class="hourtxt">时</span>
																<span class="bgtime minute">&nbsp;&nbsp;</span>
																<span class="">分</span>
																<span class="bgtime second">&nbsp;&nbsp;</span>
																<span class="">秒</span>
															</div>
															<p class="opentyle">开奖中...</p>
														</dd>
													</dl>
												</a>
											</li>

										</ul>
									</div>
								</div>

								<main>
									<div class="lottey_title">
										<ul>
											<li data-text="#hot_li" class="check">热门彩</li>
											<li data-text="#pk10_li">PK10</li>
											<li data-text="#ssc_li">时时彩</li>
											<li data-text="#check5_li">11选5</li>
											<li data-text="#klsf_li">快乐十分</li>
											<li data-text="#kuai3_li">快三</li>
											<li data-text="#other_li">其它 </li>
										</ul>
									</div>
									<div class="view_box">
										<div id="hot_li">
											<div id="pk10_hot">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/PK10/pk10kai.html"><img src="picture/beijinpk.png" alt="北京PK10" /></a>
													</div>
													<div class="num_ul">
														<h1>北京PK10 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul id="jnumber" class="numberbox">
															<li class="nub01"></li>
															<li class="nub02"></li>
															<li class="nub03"></li>
															<li class="nub04"></li>
															<li class="nub05"></li>
															<li class="nub06"></li>
															<li class="nub07"></li>
															<li class="nub08"></li>
															<li class="nub09"></li>
															<li class="nub10 li_after"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="5">1-5龙虎</th>
															<th colspan="3">冠亚和</th>
														</tr>
														<tr class="longhu">
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td class="sumFS">&nbsp;</td>
															<td class="sumBigSamll">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="pk10" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/PK10/pk10_longhulz.html">龙虎路珠</a>
														</li>
														<li>
															<a href="/view/PK10/pk10_weizhizs.html">位置走势</a>
														</li>
														<li>
															<a href="/view/PK10/pk10_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/PK10/pk10_lenrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="cqSsc_hot">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/shishicai_cq/ssc_index.html"><img src="picture/chognqingssc.png" alt="重庆时时彩" /></a>
													</div>
													<div class="num_ul">
														<h1>重庆时时彩 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="3">总和</th>
															<th>龙虎</th>
															<th>前三</th>
															<th>中三</th>
															<th>后三</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;&nbsp;</td>
															<td class="sumBigSmall">&nbsp;&nbsp;</td>
															<td class="dragonTiger">&nbsp;&nbsp;</td>
															<td class="behindThree">&nbsp;&nbsp;</td>
															<td class="betweenThree">&nbsp;&nbsp;</td>
															<td class="lastThree">&nbsp;&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="cqssc" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/shishicai_cq/ssc_luzhufenxizh.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/shishicai_cq/ssc_dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/shishicai_cq/ssc_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/shishicai_cq/ssc_lengrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="shiyix5_sd_hot">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/shiyix5_sd/index.html"><img src="picture/shandong11x5.png" alt="十一运夺金" /></a>
													</div>
													<div class="num_ul">
														<h1>十一运夺金 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="3">总和</th>
															<th>前三</th>
															<th>中三</th>
															<th>后三</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>

															<td class="behindThree">&nbsp;</td>
															<td class="betweenThree">&nbsp;</td>
															<td class="lastThree">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="sdsyydj" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/shiyix5_sd/zonghelzfx.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/shiyix5_sd/dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/shiyix5_sd/meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/shiyix5_sd/lengrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
										</div>
										<div id="pk10_li">
											<div id="pk10">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/PK10/pk10kai.html"><img src="picture/beijinpk.png" alt="北京PK10" /></a>
													</div>
													<div class="num_ul">
														<h1>北京PK10 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul id="jnumber" class="numberbox">
															<li class="nub01"></li>
															<li class="nub02"></li>
															<li class="nub03"></li>
															<li class="nub04"></li>
															<li class="nub05"></li>
															<li class="nub06"></li>
															<li class="nub07"></li>
															<li class="nub08"></li>
															<li class="nub09"></li>
															<li class="nub10 li_after"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="5">1-5龙虎</th>
															<th colspan="3">冠亚和</th>
														</tr>
														<tr class="longhu">
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td class="sumFS">&nbsp;</td>
															<td class="sumBigSamll">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="pk10" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/PK10/pk10_longhulz.html">龙虎路珠</a>
														</li>
														<li>
															<a href="/view/PK10/pk10_weizhizs.html">位置走势</a>
														</li>
														<li>
															<a href="/view/PK10/pk10_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/PK10/pk10_lenrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="jisusc">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/jisusaiche/pk10kai.html"><img src="picture/jisusaiche .png" alt="极速赛车" /></a>
													</div>
													<div class="num_ul">
														<h1>极速赛车 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul id="jnumber" class="numberbox">
															<li class="nub01"></li>
															<li class="nub02"></li>
															<li class="nub03"></li>
															<li class="nub04"></li>
															<li class="nub05"></li>
															<li class="nub06"></li>
															<li class="nub07"></li>
															<li class="nub08"></li>
															<li class="nub09"></li>
															<li class="nub10 li_after"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="5">1-5龙虎</th>
															<th colspan="3">冠亚和</th>
														</tr>
														<tr class="longhu">
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td class="sumFS">&nbsp;</td>
															<td class="sumBigSamll">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="jisusc" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/jisusaiche/pk10_longhulz.html">龙虎路珠</a>
														</li>
														<li>
															<a href="/view/jisusaiche/pk10_weizhizs.html">位置走势</a>
														</li>
														<li>
															<a href="/view/jisusaiche/pk10_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/jisusaiche/pk10_lenrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="xingyft">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/xingyft/pk10kai.html"><img src="picture/jisuft.png" alt="极速飞艇" /></a>
													</div>
													<div class="num_ul">
														<h1>极速飞艇 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul id="jnumber" class="numberbox">
															<li class="nub01"></li>
															<li class="nub02"></li>
															<li class="nub03"></li>
															<li class="nub04"></li>
															<li class="nub05"></li>
															<li class="nub06"></li>
															<li class="nub07"></li>
															<li class="nub08"></li>
															<li class="nub09"></li>
															<li class="nub10 li_after"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="5">1-5龙虎</th>
															<th colspan="3">冠亚和</th>
														</tr>
														<tr class="longhu">
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td>&nbsp;</td>
															<td class="sumFS">&nbsp;</td>
															<td class="sumBigSamll">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="xingyft" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/xingyft/pk10_longhulz.html">龙虎路珠</a>
														</li>
														<li>
															<a href="/view/xingyft/pk10_weizhizs.html">位置走势</a>
														</li>
														<li>
															<a href="/view/xingyft/pk10_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/xingyft/pk10_lenrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
										</div>
										<div id="ssc_li">
											<div id="cqSsc">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/shishicai_cq/ssc_index.html"><img src="picture/chognqingssc.png" alt="重庆时时彩" /></a>
													</div>
													<div class="num_ul">
														<h1>重庆时时彩 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="3">总和</th>
															<th>龙虎</th>
															<th>前三</th>
															<th>中三</th>
															<th>后三</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;&nbsp;</td>
															<td class="sumBigSmall">&nbsp;&nbsp;</td>
															<td class="dragonTiger">&nbsp;&nbsp;</td>
															<td class="behindThree">&nbsp;&nbsp;</td>
															<td class="betweenThree">&nbsp;&nbsp;</td>
															<td class="lastThree">&nbsp;&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="cqssc" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/shishicai_cq/ssc_luzhufenxizh.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/shishicai_cq/ssc_dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/shishicai_cq/ssc_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/shishicai_cq/ssc_lengrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="tjSsc">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/shishicai_tj/ssc_index.html"><img src="picture/tianjinssc.png" alt="天津时时彩" /></a>
													</div>
													<div class="num_ul">
														<h1>天津时时彩 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="3">总和</th>
															<th>龙虎</th>
															<th>前三</th>
															<th>中三</th>
															<th>后三</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;&nbsp;</td>
															<td class="sumBigSmall">&nbsp;&nbsp;</td>
															<td class="dragonTiger">&nbsp;&nbsp;</td>
															<td class="behindThree">&nbsp;&nbsp;</td>
															<td class="betweenThree">&nbsp;&nbsp;</td>
															<td class="lastThree">&nbsp;&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="tjssc" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/shishicai_tj/ssc_luzhufenxizh.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/shishicai_tj/ssc_dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/shishicai_tj/ssc_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/shishicai_tj/ssc_lengrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="xjSsc">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/shishicai_xj/ssc_index.html"><img src="picture/xinjiangssc.png" alt="新疆时时彩" /></a>
													</div>
													<div class="num_ul">
														<h1>新疆时时彩 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="3">总和</th>
															<th>龙虎</th>
															<th>前三</th>
															<th>中三</th>
															<th>后三</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;&nbsp;</td>
															<td class="sumBigSmall">&nbsp;&nbsp;</td>
															<td class="dragonTiger">&nbsp;&nbsp;</td>
															<td class="behindThree">&nbsp;&nbsp;</td>
															<td class="betweenThree">&nbsp;&nbsp;</td>
															<td class="lastThree">&nbsp;&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="xjssc" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/shishicai_xj/ssc_luzhufenxizh.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/shishicai_xj/ssc_dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/shishicai_xj/ssc_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/shishicai_xj/ssc_lengrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
										</div>
										<div id="check5_li">
											<div id="shiyix5_sd">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/shiyix5_sd/index.html"><img src="picture/shandong11x5.png" alt="十一运夺金" /></a>
													</div>
													<div class="num_ul">
														<h1>十一运夺金 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="3">总和</th>
															<th>前三</th>
															<th>中三</th>
															<th>后三</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>

															<td class="behindThree">&nbsp;</td>
															<td class="betweenThree">&nbsp;</td>
															<td class="lastThree">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="sdsyydj" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/shiyix5_sd/zonghelzfx.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/shiyix5_sd/dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/shiyix5_sd/meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/shiyix5_sd/lengrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="shiyix5_gd">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/shiyix5_gd/index.html"><img src="picture/gaungdong11x5.png" alt="广东11选5" /></a>
													</div>
													<div class="num_ul">
														<h1>广东11选5 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="3">总和</th>
															<th>前三</th>
															<th>中三</th>
															<th>后三</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>

															<td class="behindThree">&nbsp;</td>
															<td class="betweenThree">&nbsp;</td>
															<td class="lastThree">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="gdsyxw" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/shiyix5_gd/zonghelzfx.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/shiyix5_gd/dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/shiyix5_gd/meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/shiyix5_gd/lengrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="11ckeck5_zjef">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/shiyix5_zhejiang/index.html"><img src="picture/zj11x5.png" alt="浙江11选5" /></a>
													</div>
													<div class="num_ul">
														<h1>浙江11选5 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="3">总和</th>
															<th>前三</th>
															<th>中三</th>
															<th>后三</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>

															<td class="behindThree">&nbsp;</td>
															<td class="betweenThree">&nbsp;</td>
															<td class="lastThree">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="zjef" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/shiyix5_zhejiang/zonghelzfx.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/shiyix5_zhejiang/dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/shiyix5_zhejiang/meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/shiyix5_zhejiang/lengrefx.html">冷热分析</a>
														</li>
													</ul>
												</div>
											</div>
										</div>
										<div id="klsf_li">
											<div id="gdklsf">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/klsf/klsf_index.html"><img src="picture/guangdongklsf.png" alt="广东快乐十分" /></a>
													</div>
													<div class="num_ul">
														<h1>广东快乐十分 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="4">总和</th>
															<th colspan="4">1-4龙虎</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="lastBigSmall">&nbsp;</td>

															<td class="firstDragonTiger">&nbsp;</td>
															<td class="secondDragonTiger">&nbsp;</td>
															<td class="thirdDragonTiger">&nbsp;</td>
															<td class="fourthDragonTiger">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="gdklsf" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/klsf/klsf_zonghelzfx.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/klsf/klsf_jibenzs.html">号码分布</a>
														</li>
														<li>
															<a href="/view/klsf/klsf_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/klsf/klsf_daxiaozs.html">大小走势</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="tjklsf">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/klsf_tianjin/klsf_index.html"><img src="picture/tjklsf.png" alt="天津快乐十分" /></a>
													</div>
													<div class="num_ul">
														<h1>天津快乐十分 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="4">总和</th>
															<th colspan="4">1-4龙虎</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="lastBigSmall">&nbsp;</td>

															<td class="firstDragonTiger">&nbsp;</td>
															<td class="secondDragonTiger">&nbsp;</td>
															<td class="thirdDragonTiger">&nbsp;</td>
															<td class="fourthDragonTiger">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="tjklsf" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/klsf_tianjin/klsf_zonghelzfx.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/klsf_tianjin/klsf_jibenzs.html">号码分布</a>
														</li>
														<li>
															<a href="/view/klsf_tianjin/klsf_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/klsf_tianjin/klsf_daxiaozs.html">大小走势</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="aozxy8">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/aozxy8/klsf_index.html"><img src="picture/aozxy8.png" alt="澳洲幸运8" /></a>
													</div>
													<div class="num_ul">
														<h1>澳洲幸运8 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="4">总和</th>
															<th colspan="4">1-4龙虎</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="lastBigSmall">&nbsp;</td>

															<td class="firstDragonTiger">&nbsp;</td>
															<td class="secondDragonTiger">&nbsp;</td>
															<td class="thirdDragonTiger">&nbsp;</td>
															<td class="fourthDragonTiger">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="aozxy8" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/aozxy8/klsf_zonghelzfx.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/aozxy8/klsf_jibenzs.html">号码分布</a>
														</li>
														<li>
															<a href="/view/aozxy8/klsf_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/aozxy8/klsf_daxiaozs.html">大小走势</a>
														</li>
													</ul>
												</div>
											</div>
										</div>
										<div id="kuai3_li">
											<div id="kuai3">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/kuai3/kuai3_index.html"><img src="picture/jsk3.png" alt="江苏快3" /></a>
													</div>
													<div class="num_ul">
														<h1>江苏快3 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="num1"></li>
															<li class="num2"></li>
															<li class="num4"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="3">总和</th>
															<th colspan="3">鱼虾蟹</th>
														</tr>
														<tr>
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="firstSeafood">&nbsp;</td>
															<td class="secondSeafood">&nbsp;</td>
															<td class="thirdSeafood">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="jsksan" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/kuai3/kuai3_zonghelz.html">总和路珠</a>
														</li>
														<li>
															<a href="/view/kuai3/kuai3_dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/kuai3/kuai3_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/kuai3/kuai3_haomalz.html">号码路珠</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="gxk3">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/kuai3_guangxi/kuai3_index.html"><img src="picture/gxk3.png" alt="广西快3" /></a>
													</div>
													<div class="num_ul">
														<h1>广西快3 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="num1"></li>
															<li class="num2"></li>
															<li class="num4"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="3">总和</th>
															<th colspan="3">鱼虾蟹</th>
														</tr>
														<tr>
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="firstSeafood">&nbsp;</td>
															<td class="secondSeafood">&nbsp;</td>
															<td class="thirdSeafood">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="gxft" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/kuai3_guangxi/kuai3_zonghelz.html">总和路珠</a>
														</li>
														<li>
															<a href="/view/kuai3_guangxi/kuai3_dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/kuai3_guangxi/kuai3_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/kuai3_guangxi/kuai3_haomalz.html">号码路珠</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="bjk3">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/kuai3_beijing/kuai3_index.html"><img src="picture/bjkl3.png" alt="北京快3" /></a>
													</div>
													<div class="num_ul">
														<h1>北京快3 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="num1"></li>
															<li class="num2"></li>
															<li class="num4"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="3">总和</th>
															<th colspan="3">鱼虾蟹</th>
														</tr>
														<tr>
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="firstSeafood">&nbsp;</td>
															<td class="secondSeafood">&nbsp;</td>
															<td class="thirdSeafood">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="bjft" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/kuai3_beijing/kuai3_zonghelz.html">总和路珠</a>
														</li>
														<li>
															<a href="/view/kuai3_beijing/kuai3_dinweizs.html">定位走势</a>
														</li>
														<li>
															<a href="/view/kuai3_beijing/kuai3_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/kuai3_beijing/kuai3_haomalz.html">号码路珠</a>
														</li>
													</ul>
												</div>
											</div>
										</div>
										<div id="other_li">
											<div id="cqnc">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/cqnc/index.html"><img src="picture/chongqingnc.png" alt="重庆农场" /></a>
													</div>
													<div class="num_ul">
														<h1>重庆农场 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="ncnum01"></li>
															<li class="ncnum02"></li>
															<li class="ncnum03"></li>
															<li class="ncnum04"></li>
															<li class="ncnum05"></li>
															<li class="ncnum11"></li>
															<li class="ncnum18"></li>
															<li class="ncnum16"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th width="40" colspan="4">总和</th>
															<th colspan="4">1-4龙虎</th>
														</tr>
														<tr class="longhu2">
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
															<td class="lastBigSmall">&nbsp;</td>

															<td class="firstDragonTiger">&nbsp;</td>
															<td class="secondDragonTiger">&nbsp;</td>
															<td class="thirdDragonTiger">&nbsp;</td>
															<td class="fourthDragonTiger">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul>
														<li>
															<a href="javascript:void(0)" data-text="cqxync" class="video_a">开奖视频</a>
														</li>
														<li>
															<a href="/view/cqnc/klsf_zonghelzfx.html">路珠分析</a>
														</li>
														<li>
															<a href="/view/cqnc/klsf_daxiaozs.html">大小走势</a>
														</li>
														<li>
															<a href="/view/cqnc/klsf_meiricltj.html">长龙提醒</a>
														</li>
														<li>
															<a href="/view/cqnc/klsf_jibenzs.html">号码分布</a>
														</li>
													</ul>
												</div>
											</div>
											<!--<div id="qxc">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="view/tc7xc/index.html"><img src="picture/qxc.png" /></a>
													</div>
													<div class="num_ul">
														<h1>七星彩 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numWeightblue "></li>
															<li class="numWeightblue "></li>
															<li class="numWeightblue "></li>
															<li class="numblue numLightblue"></li>
															<li class="numblue numLightblue"></li>
															<li class="numblue numLightblue"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="3">总和</th>
														</tr>
														<tr>
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul class="width25">
														<li>
															<a href="javascript:void(0)" data-text="qxc" class="video_a">开奖视频</a>
														</li>
														<!--<li>
															<a href="javascript:void(0)">龙虎路珠</a>
														</li>
														<li>
															<a href="javascript:void(0)">位置走势</a>
														</li>
														<li>
															<a href="javascript:void(0)">长龙提醒</a>
														</li>
														<li>
															<a href="javascript:void(0)">冷热分析</a>
														</li>->/////////
													</ul>
												</div>
											</div>
											<div id="pcdd">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="view/PC_egxy28/PC_egxy28index.html"><img src="picture/egxy28.png" /></a>
													</div>
													<div class="num_ul">
														<h1>PC蛋蛋28 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
															<li class="numblueHead"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="3">总和</th>
														</tr>
														<tr>
															<td class="sumNum">&nbsp;</td>
															<td class="sumSingleDouble">&nbsp;</td>
															<td class="sumBigSmall">&nbsp;</td>
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul class="width25">
														<li>
															<a href="javascript:void(0)" data-text="egxy28" class="video_a">开奖视频</a>
														</li>
														<!--<li>
															<a href="javascript:void(0)">龙虎路珠</a>
														</li>
														<li>
															<a href="javascript:void(0)">位置走势</a>
														</li>
														<li>
															<a href="javascript:void(0)">长龙提醒</a>
														</li>
														<li>
															<a href="javascript:void(0)">冷热分析</a>

														</li>->////////
													</ul>
												</div>
											</div>-->
											<div id="tcpl3">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/tcpl3/index.html"><img src="picture/pailie3 .png" alt="体彩排列3" /></a>
													</div>
													<div class="num_ul">
														<h1>体彩排列3 <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="num_red"></li>
															<li class="num_red"></li>
															<li class="num_red"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="3">佰拾和</th>
															<th colspan="3">佰个和</th>
															<th colspan="3">拾个和</th>
															<th colspan="3">总和</th>
														</tr>
														<tr class="tabTr">
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul class="width25">
														<li>
															<a href="javascript:void(0)" data-text="pailie3" class="video_a">开奖视频</a>
														</li>
													</ul>
												</div>
											</div>
											<div id="fc3d">
												<div class="rowbox1">
													<div class="imgheadl">
														<a href="/view/fc3D/index.html"><img src="picture/fc3d .png" alt="福彩3D" /></a>
													</div>
													<div class="num_ul">
														<h1>福彩3D <span class="preDrawIssue"> 223366</span> 期</h1>
														<p>当前<span class="drawCount">024</span>期，剩<span class="sdrawCountnext">2424</span>期</p>
														<ul class="kajianhao">
															<li class="num_red"></li>
															<li class="num_red"></li>
															<li class="num_red"></li>
														</ul>
													</div>
													<div class="nextkai_time">
														<p>距下期开奖</p>
														<div class="line linetime cuttime indextime" id="timebox">
															<span class="bgtime hour">09</span>
															<span class="hourtxt">时</span>
															<span class="bgtime minute">26</span>
															<span>分</span>
															<span class="bgtime second">56</span>
															<span>秒</span>
														</div>
														<p class="opentyle">开奖中...</p>
													</div>
												</div>
												<div class="rowbox2">
													<table border="0" cellpadding="1" cellspacing="1">
														<tr>
															<th colspan="3">佰拾和</th>
															<th colspan="3">佰个和</th>
															<th colspan="3">拾个和</th>
															<th colspan="3">总和</th>
														</tr>
														<tr class="tabTr">
														</tr>
													</table>
												</div>
												<div class="rowbox3" id="rowbox3">
													<ul class="width25">
														<li>
															<a href="javascript:void(0)" data-text="fcsd" class="video_a">开奖视频</a>
														</li>
													</ul>
												</div>
											</div>
										</div>
									</div>

									<div class="px10sanhao">
										<div class="sahu_lottey">
											<h1>北京PK拾 <span> 杀号计划</span></h1>
										</div>
										<p class="rank">
											<span class="active" data-text="fifth">冠军</span>
											<span data-text="second">亚军</span>
											<span data-text="third">第三名</span>
											<span data-text="fourth">第四名</span>
											<span data-text="fifth">第五名</span>
											<span data-text="sixth">第六名</span>
											<span data-text="seven">第七名</span>
											<span data-text="eight">第八名</span>
											<span data-text="nine">第九名</span>
											<span data-text="ten">第十名</span>
										</p>
										<div class="user_score">
											<ul>
												<!--<li>
													<div class="head_div">
														<img src="picture/headicon_2.png" alt="" />
														<span>狂神杀号</span>
													</div>
													<div class="result">
														<h3>最新冠军杀号: <span>5</span></h3>
														<p>近10期成功率： <span class="clo_red">80%</span></p>
														<p>历史战绩： <span>80%</span></p>
													</div>
												</li>-->

											</ul>
										</div>
									</div>

									<div class="SSCsanhao">
										<div class="sahu_lottey">
											<h1>重庆时时彩 <span> 杀号计划</span></h1>
										</div>
										<p class="rank">
											<span class="active" data-text="fifth">万位</span>
											<span data-text="second">千位</span>
											<span data-text="third">百位</span>
											<span data-text="fourth">十位</span>
											<span data-text="fifth">个位</span>
										</p>
										<div class="user_score">
											<ul>
												<li>
													<div class="head_div">
														<img src="picture/headicon_2.png" alt="" />
														<span>狂神杀号</span>
													</div>
													<div class="result">
														<h3>最新冠军杀号: <span>5</span></h3>
														<p>近10期成功率： <span class="clo_red">80%</span></p>
														<p>历史战绩： <span>80%</span></p>
													</div>
												</li>

											</ul>
										</div>
									</div>

								</main>

								<aside>
									<div class="announce_down">
										<h2>推荐彩种 <span clss="tjspan">RECOMMEND LOTTERY</span></h2>
										<div class="tjlottey" id="哈哈哈哈哈">
											<ul>

												<li>
													<a target="_blank" href="/view/PK10/pk10kai.html"><img src="picture/bjpk10.png" alt="北京PK10" /></a>
													<a target="_blank" href="/view/PK10/pk10kai.html">北京PK10</a>
												</li>

												<li>
													<a target="_blank" href="/view/jisusaiche/pk10kai.html"><img src="picture/jisu_saiche.png" alt="极速赛车" /></a>
													<a target="_blank" href="/view/jisusaiche/pk10kai.html">极速赛车</a>
												</li>
												<li>
													<a target="_blank" href="/view/aozxy5/ssc_index.html"><img src="picture/aozxy5.png" alt="澳洲幸运5" /></a>
													<a target="_blank" href="/view/aozxy5/ssc_index.html">澳洲幸运5</a>
												</li>
												<li>
													<a target="_blank" href="/view/PC_egxy28/PC_egxy28index.html"><img src="picture/pcdd_1.png" alt="PC蛋蛋28" /></a>
													<a target="_blank" href="/view/PC_egxy28/PC_egxy28index.html">PC蛋蛋28</a>
												</li>
												<li>
													<a target="_blank" href="/view/cqnc/index.html"><img src="picture/cqnc.png" alt="重庆幸运农场" /></a>
													<a target="_blank" href="/view/cqnc/index.html">重庆幸运农场</a>
												</li>

												<li>
													<a target="_blank" href="/view/klsf/klsf_index.html"><img src="picture/gdklsf.png" alt="广东快乐十分" /></a>
													<a target="_blank" href="/view/klsf/klsf_index.html">广东快乐十分</a>
												</li>

												<li>
													<a target="_blank" href="/view/shiyix5_jiangxi/index.html"><img src="picture/11x5_jiangxi.png" alt="江西11选5" /></a>
													<a target="_blank" href="/view/shiyix5_jiangxi/index.html">江西11选5</a>
												</li>

												<li class="first_li">
													<a target="_blank" href="/view/shishicai_jisu/ssc_index.html"><img src="picture/ssc_jisu.png" alt="极速时时彩" /></a>
													<a target="_blank" href="/view/shishicai_jisu/ssc_index.html">极速时时彩</a>
												</li>

												<li>
													<a target="_blank" href="/view/shishicai_cq/ssc_index.html"><img src="picture/cqssc.png" alt="重庆时时彩" /></a>
													<a target="_blank" href="/view/shishicai_cq/ssc_index.html">重庆时时彩</a>
												</li>
												<li>
													<a target="_blank" href="/view/shiyix5_sd/index.html"><img src="picture/syydj.png" alt="十一运夺金" /></a>
													<a target="_blank" href="/view/shiyix5_sd/index.html">十一运夺金</a>
												</li>
												<li>
													<a target="_blank" href="/view/shishicai_tj/ssc_index.html"><img src="picture/tjssc.png" alt="天津时时彩" /></a>
													<a target="_blank" href="/view/shishicai_tj/ssc_index.html">天津时时彩</a>
												</li>

												<li class="last_li">
													<a target="_blank" href="/view/aozxy10/pk10kai.html"><img src="picture/aozxy10.png" alt="澳洲幸运10" /></a>
													<a target="_blank" href="/view/aozxy10/pk10kai.html">澳洲幸运10</a>
												</li>

											</ul>
											<span class="movedowm"><span></span></span>
										</div>
									</div>

									<div class="jiahu">
										<ul>
											<li>
												<a href="/view/zsh/index.html?type=pk10" class="pkxl">
													<div class="bgdiv bigbg">
														<div>
															<h2>PK10系列</h2>
															<p><span>购彩计划</span><span>稳赢杀号</span></p>
														</div>
													</div>
												</a>
											</li>
											<li class="havemargin">
												<a href="/view/zsh/index.html?type=cqssc" class="sscxl">
													<div class="bgdiv">
														<div>
															<h2>时时彩系列</h2>
															<p><span>购彩计划</span><span>稳赢杀号</span></p>
														</div>
													</div>
												</a>
											</li>
											<li>
												<a href="/view/zsh/index.html?type=gdsyxw" class="check5xl">
													<div class="bgdiv">
														<div>
															<h2>11选5系列</h2>
															<p><span>购彩计划</span><span>稳赢杀号</span></p>
														</div>
													</div>
												</a>
											</li>
											<li class="havemargin">
												<a href="/view/zsh/index.html?type=jsksan" class="k3xl">
													<div class="bgdiv">
														<div>
															<h2>快3系列</h2>
															<p><span>购彩计划</span><span>稳赢杀号</span></p>
														</div>
													</div>
												</a>
											</li>
											<li>
												<a href="/view/zsh/index.html?type=egxy28" class="pcddxl">
													<div class="bgdiv">
														<div>
															<h2>PC蛋蛋系列</h2>
															<p><span>购彩计划</span><span>稳赢杀号</span></p>
														</div>
													</div>
												</a>
											</li>
										</ul>
									</div>
									<div class="otherr">
										<div class="program">
											<dl>
												<dt>
											方案预测
										</dt>
												<dd>
													<a href="news/list-zx.html">更多</a>
												</dd>
											</dl>
										</div>
										<div class="newslist">
											<ul id="fanganyc">
												<input type="hidden" id="fanganycdata" value="[{&#34;releaseDate&#34;:&#34;2018-08-17&#34;,&#34;image&#34;:&#34;87bd8d87-c998-4a1f-8df8-907b6700f642.jpg&#34;,&#34;title&#34;:&#34;江苏快3推荐（08.17）：上午推荐&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:425,&#34;newsId&#34;:2743,&#34;description&#34;:&#34;关注的号码：2.4.4；1.4.5；2.4.5；2.4.6(追号期数为：第4-29期)&#34;},{&#34;releaseDate&#34;:&#34;2018-08-17&#34;,&#34;image&#34;:&#34;0eb41e24-5b9c-48e5-83a2-e7d521aa5df6.png&#34;,&#34;title&#34;:&#34;安徽快3推荐（08.17）：上午推荐&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:99,&#34;newsId&#34;:2742,&#34;description&#34;:&#34;关注的号码：2.3.4；2.4.4；2.3.5；2.4.5；2.3.6；2.5.5；2.4.6；2.5.6(追号期数为：第4-29期)&#34;},{&#34;releaseDate&#34;:&#34;2018-08-17&#34;,&#34;image&#34;:&#34;b7c92170-38fe-4ab9-846d-9e1c84f8e2eb.jpg&#34;,&#34;title&#34;:&#34;湖北快3推荐（08.17）：上午推荐&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:117,&#34;newsId&#34;:2741,&#34;description&#34;:&#34;关注的号码：1.4.5；2.4.5(追号期数为：第4-29期)&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;ff728c06-471b-4300-812b-e4c87bbe1d1e.jpg&#34;,&#34;title&#34;:&#34;江西11选5号码推荐8月16日下午&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:121,&#34;newsId&#34;:2738,&#34;description&#34;:&#34;根据近期走势图看，04,02码较热，06,03码较冷，彩民可以持续关注热码，防止冷码回补。&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;052f21a0-536e-4b19-8882-ea78592c4cf6.jpg&#34;,&#34;title&#34;:&#34;湖北11选5号码推荐8月16日下午&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:111,&#34;newsId&#34;:2737,&#34;description&#34;:&#34;根据近期走势图看，05,06码较热，07,11码较冷，彩民可以持续关注热码，防止冷码回补。&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;36094da9-1fac-453c-b5f4-17f6a375e07c.jpg&#34;,&#34;title&#34;:&#34;浙江11选5号码推荐8月16日下午&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:96,&#34;newsId&#34;:2736,&#34;description&#34;:&#34;根据近期走势图看，06,03码较热，05,08码较冷，彩民可以持续关注热码，防止冷码回补。&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;d1c3189c-0abb-4c78-98c1-fe842102021e.jpg&#34;,&#34;title&#34;:&#34;双色球第18095期预测：看好奇数走强&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:120,&#34;newsId&#34;:2733,&#34;description&#34;:&#34;彩票开奖&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;0923d935-3377-487e-b24e-7fd0963bf239.jpg&#34;,&#34;title&#34;:&#34;福彩3D第18220期分析：关注三胆538&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:127,&#34;newsId&#34;:2732,&#34;description&#34;:&#34;彩票开奖&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;0957e5b7-3508-46f7-a9af-b27a792a2b10.jpg&#34;,&#34;title&#34;:&#34;湖北11选5号码推荐8月16日上午&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:89,&#34;newsId&#34;:2731,&#34;description&#34;:&#34;根据近期走势图看，10,06码较热，04,01码较冷，彩民可以持续关注热码，防止冷码回补。&#34;},{&#34;releaseDate&#34;:&#34;2018-08-16&#34;,&#34;image&#34;:&#34;0c89e834-014e-4e62-9683-b8a677ef860b.jpg&#34;,&#34;title&#34;:&#34;江西11选5号码推荐8月16日上午&#34;,&#34;labels&#34;:&#34;&#34;,&#34;pageView&#34;:99,&#34;newsId&#34;:2730,&#34;description&#34;:&#34;根据近期走势图看，08,02码较热，06,03码较冷，彩民可以持续关注热码，防止冷码回补。&#34;}]">
												
												<li>
													<a target="_blank" href="news/detail-2743.html">
														江苏快3推荐（08.17）：上午推荐
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2742.html">
														安徽快3推荐（08.17）：上午推荐
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2741.html">
														湖北快3推荐（08.17）：上午推荐
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2738.html">
														江西11选5号码推荐8月16日下午
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2737.html">
														湖北11选5号码推荐8月16日下午
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2736.html">
														浙江11选5号码推荐8月16日下午
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2733.html">
														双色球第18095期预测：看好奇数走强
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2732.html">
														福彩3D第18220期分析：关注三胆538
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2731.html">
														湖北11选5号码推荐8月16日上午
													</a>
												</li>
												
												<li>
													<a target="_blank" href="news/detail-2730.html">
														江西11选5号码推荐8月16日上午
													</a>
												</li>
												
											</ul>
										</div>
										<div class="adviertisement">
											<a href="/view/PK10/pk10kai.html" target="_blank">
												<img src="picture/adviertisement.png" alt="" />
											</a>
										</div>
										<div class="tools">
											<div class="toolhead">
												<dl>
													<dt>
												分析工具
											</dt>
												</dl>
											</div>
											<div class="toolsbox">
												<div class="toolslist">
													<div class="listhead">
														<div class="toolsr1">
															<div class="tooll">
																<img src="picture/beijinpk10.png" alt="北京pk10" />
															</div>
															<div class="toolr">
																北京pk10
															</div>
														</div>
														<div class="toolsr2">
															<dl>
																<dt class="toolicon1"><i></i>走势</dt>
																<dd>
																	<a href="/view/PK10/pk10kai.html">更多</a>
																</dd>
															</dl>
														</div>
														<div class="toolsr3">
															<ul>
																<li>
																	<a href="/view/PK10/pk10_guanyahezs.html">冠亚和走势</a>
																</li>
																<li>
																	<a href="/view/PK10/pk10_weizhizs.html">位置走势</a>
																</li>
																<li>
																	<a href="/view/PK10/pk10_haomazs.html">号码走势</a>
																</li>
															</ul>
														</div>

														<div class="toolsr2 toolborder">
															<dl>
																<dt class="toolicon1"><i></i>遗漏</dt>
																<dd>
																	<a href="/view/PK10/pk10kai.html">更多</a>
																</dd>
															</dl>
														</div>
														<div class="toolsr3">
															<ul>
																<li>
																	<a href="/view/PK10/pk10_guanyahezs.html">冠亚和</a>
																</li>
																<li>
																	<a href="/view/PK10/pk10_weizhizs.html">冠军遗漏</a>
																</li>
																<li>
																	<a href="/view/PK10/pk10_weizhizs.html?second">亚军遗漏</a>
																</li>
																<li>
																	<a href="/view/PK10/pk10_weizhizs.html?third">第三名遗漏</a>
																</li>
																<li>
																	<a href="/view/PK10/pk10_weizhizs.html?fourth">第四名遗漏</a>
																</li>
															</ul>
														</div>
														<!--<div class="toolsr2">
													<dl>
														<dt class="toolicon2"><i></i>遗漏</dt>
														<dd>
															<a href="">更多</a>
														</dd>
													</dl>
												</div>
												<div class="toolsr3 noboder">
													<ul>
														<li>冠亚和 </li>
														<li>冠军遗漏</li>
														<li>亚军遗漏</li>
														<li>第三名遗漏</li>
														<li>第四名遗漏</li>
														<li>第五名遗漏</li>
													</ul>
												</div>-->
													</div>
												</div>
												<div class="toolslist">
													<div class="listhead">
														<div class="toolsr1">
															<div class="tooll">
																<img src="picture/chongqingssc.png" alt="重庆时时彩" />
															</div>
															<div class="toolr">
																重庆时时彩
															</div>
														</div>
														<div class="toolsr2">
															<dl>
																<dt class="toolicon1"><i></i>走势</dt>
																<dd>
																	<a href="/view/shishicai_cq/ssc_index.html">更多</a>
																</dd>
															</dl>
														</div>
														<div class="toolsr3">
															<ul>
																<li>
																	<a href="/view/shishicai_cq/ssc_jibenzs.html">基本走势</a>
																</li>
																<li>
																	<a href="/view/shishicai_cq/ssc_longhuzs.html">龙虎走势</a>
																</li>
																<li>
																	<a href="/view/shishicai_cq/ssc_xingtaizs.html">形态走势</a>
																</li>
															</ul>
														</div>
														<div class="toolsr2 toolborder">
															<dl>
																<dt class="toolicon1"><i></i>遗漏</dt>
																<dd>
																	<a href="/view/shishicai_cq/ssc_index.html">更多</a>
																</dd>
															</dl>
														</div>
														<div class="toolsr3">
															<ul>
																<li>
																	<a href="/view/shishicai_cq/ssc_zhonghefx.html">综合分析</a>
																</li>
																<li>
																	<a href="/view/shishicai_cq/ssc_dinweizs.html">定位走势</a>
																</li>
																<li>
																	<a href="/view/shishicai_cq/ssc_dinweizs.html?second">第二球走势</a>
																</li>
																<li>
																	<a href="/view/shishicai_cq/ssc_dinweizs.html?third">第三球走势</a>
																</li>
																<li>
																	<a href="/view/shishicai_cq/ssc_dinweizs.html?fourth">第四球走势</a>
																</li>
															</ul>
														</div>
														<!--<div class="toolsr2">
													<dl>
														<dt class="toolicon2"><i></i>遗漏</dt>
														<dd>
															<a href="">更多</a>
														</dd>
													</dl>
												</div>
												<div class="toolsr3 noboder">
													<ul>
														<li>冠亚和 </li>
														<li>冠军遗漏</li>
														<li>亚军遗漏</li>
														<li>第三名遗漏</li>
														<li>第四名遗漏</li>
														<li>第五名遗漏</li>
													</ul>
												</div>-->
													</div>
												</div>
												<!--<div class="toolslist">
											<div class="listhead">
												<div class="toolsr1">
													<div class="tooll">
														<img src="picture/klsf.png" />
													</div>
													<div class="toolr">
														快乐十分
													</div>
												</div>
												<div class="toolsr2">
													<dl>
														<dt class="toolicon1"><i></i>走势</dt>
														<dd>
															<a href="view/klsf/klsf_index.html">更多</a>
														</dd>
													</dl>
												</div>
												<div class="toolsr3">
													<ul>
														<li>
															<a href="view/klsf/klsf_jibenzs.html">基本走势</a>
														</li>
														<li>
															<a href="view/klsf/klsf_danshaungzs.html">单双走势</a>
														</li>
													</ul>
												</div>
												<div class="toolsr2">
													<dl>
														<dt class="toolicon2"><i></i>遗漏</dt>
														<dd>
															<a href="">更多</a>
														</dd>
													</dl>
												</div>
												<div class="toolsr3 noboder">
													<ul>
														<li>冠亚和 </li>
														<li>冠军遗漏</li>
														<li>亚军遗漏</li>
														<li>第三名遗漏</li>
														<li>第四名遗漏</li>
														<li>第五名遗漏</li>
													</ul>
												</div>
											</div>
										</div>-->
												<!--<div class="toolslist">
											<div class="listhead">
												<div class="toolsr1">
													<div class="tooll">
														<img src="picture/jsk3_1.png" />
													</div>
													<div class="toolr">
														江苏快3
													</div>
												</div>
												<div class="toolsr2">
													<dl>
														<dt class="toolicon1"><i></i>走势</dt>
														<dd>
															<a href="view/kuai3/kuai3_index.html">更多</a>
														</dd>
													</dl>
												</div>
												<div class="toolsr3">
													<ul>
														<li>
															<a href="view/kuai3/kuai3_hezhizs.html">和值走势</a>
														</li>
														<li>
															<a href="view/kuai3/kuai3_daxiaozs.html">大小走势</a>
														</li>
														<li>
															<a href="view/kuai3/kuai3_jiouzs.html">奇偶走势</a>
														</li>
													</ul>
												</div>
												<!--<div class="toolsr2">
													<dl>
														<dt class="toolicon2"><i></i>遗漏</dt>
														<dd>
															<a href="">更多</a>
														</dd>
													</dl>
												</div>
												<div class="toolsr3 noboder">
													<ul>
														<li>冠亚和 </li>
														<li>冠军遗漏</li>
														<li>亚军遗漏</li>
														<li>第三名遗漏</li>
														<li>第四名遗漏</li>
														<li>第五名遗漏</li>
													</ul>
												</div>
											</div>
										</div>-->
											</div>
										</div>
									</div>
								</aside>
								<!--<div class="kair_right">
								<div><img class="phone" src="img/Revision_img/168phone.png?v=201889947" alt="" /></div>
								<div><img class="phone_type" src="picture/phone_type.png" alt="" /></div>
								<h1>扫一扫，下载移动客户端</h1>
								<span>（IOS、Android均可扫码下载）</h1>
							</div>-->
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="contentbox">
				<div class="contentc">

					<!--动画区-->
					<div class="contentcb">
						<div class="" style="overflow: hidden; height: auto;min-height: 1406px;">
							<div class="cztypel">
								<ul>
									<li class="cztypell" id="pk10">
										<div class="cztypelt">
											<div class="rowbox1">
												<input type="hidden" id="ofNoLoad" value="noLoad" />
												<input type="hidden" id="ifindex" value="index" />
												<div class="imgheadl">
													<a href="/view/PK10/pk10kai.html"><img src="picture/beijinpk.png" alt="北京PK10" /></a>
												</div>
												<div class="inforr">
													<h1>北京PK10第<span style="display: inline-block;"  class="preDrawIssue"></span>期开奖</h1>
													<p>每5分钟一期，全天179期</p>
													</br>
													<span class="nextIssue displaynone"></span>
													<p>当前<span class="drawCount"></span>期，剩余<span class="sdrawCountnext"></span>期</p>
													<p class="opentyle">开奖中...</p>
													<div class="line linetime cuttime indextime" id="timebox">
														<span class="bgtime hour">...</span>
														<span class="hourtxt">时</span>
														<span class="bgtime minute">...</span>
														<span>分</span>
														<span class="bgtime second">...</span>
														<span>秒</span>
													</div>
												</div>
											</div>
											<div class="rowbox2">
												<ul id="jnumber" class="numberbox">
													<li class="nub01"></li>
													<li class="nub02"></li>
													<li class="nub03"></li>
													<li class="nub04"></li>
													<li class="nub05"></li>
													<li class="nub06"></li>
													<li class="nub07"></li>
													<li class="nub08"></li>
													<li class="nub09"></li>
													<li class="nub10 li_after"></li>
												</ul>
											</div>
											<div class="rowbox3" id="rowbox3">
												<table border="0" cellpadding="1" cellspacing="1">
													<tr>
														<th colspan="5">1-5龙虎</th>
														<th colspan="3">冠亚和</th>
													</tr>
													<tr class="longhu">
														<td>&nbsp;</td>
														<td>&nbsp;</td>
														<td>&nbsp;</td>
														<td>&nbsp;</td>
														<td>&nbsp;</td>
														<td class="sumFS">&nbsp;</td>
														<td class="sumBigSamll">&nbsp;</td>
														<td class="sumSingleDouble">&nbsp;</td>
													</tr>
												</table>
											</div>
										</div>
										<div class="cztypelb">
											<table border="0" cellpadding="1" cellspacing="1">
												<tr>
													<td>
														<a href="/view/PK10/pk10kai_history.html">历史开奖</a>
													</td>
													<td>
														<a href="/view/PK10/pk10kai_luzhufxzh.html">路珠分析</a>
													</td>
													<td>
														<a href="/view/PK10/pk10_lenrefx.html">冷热分析</a>
													</td>
													<td>
														<a href="/view/PK10/pk10_guanyahezs.html">冠亚和走势</a>
													</td>
												</tr>
											</table>
										</div>
									</li>
									<li class="cztypell" id="cqSsc">
										<div class="cztypelt">
											<div class="rowbox1">
												<div class="imgheadl">
													<a href="/view/shishicai_cq/ssc_index.html"><img src="picture/chognqingssc.png" alt="重庆时时彩" /></a>
												</div>
												<div class="inforr">
													<span class="nextIssue displaynone"></span>
													<h1>重庆时时彩第<span class="preDrawIssue"></span>期开奖</h1>
													<p>10:00-22:00，十分钟一期</p>
													<p>22:00-02:00，五分钟一期，全天120期</p>
													<p>当前<span class="drawCount"></span>期，剩余<span class="sdrawCountnext"></span>期</p>
													<p class="opentyle">开奖中...</p>
													<div class="line linetime cuttime indextime">
														<span class="bgtime hour">...</span>
														<span class="hourtxt">时</span>
														<span class="bgtime minute">...</span>
														<span>分</span>
														<span class="bgtime second">...</span>
														<span>秒</span>
													</div>
												</div>
											</div>
											<div class="rowbox2">
												<ul class="kajianhao">
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
												</ul>
											</div>
											<div class="rowbox3">
												<table border="0" cellpadding="1" cellspacing="1">
													<tr>
														<th width="40" colspan="3">总和</th>
														<th>龙虎</th>
														<th>前三</th>
														<th>中三</th>
														<th>后三</th>
													</tr>
													<tr class="longhu2">
														<td class="sumNum">&nbsp;&nbsp;</td>
														<td class="sumSingleDouble">&nbsp;&nbsp;</td>
														<td class="sumBigSmall">&nbsp;&nbsp;</td>
														<td class="dragonTiger">&nbsp;&nbsp;</td>
														<td class="behindThree">&nbsp;&nbsp;</td>
														<td class="betweenThree">&nbsp;&nbsp;</td>
														<td class="lastThree">&nbsp;&nbsp;</td>
													</tr>
												</table>
											</div>
										</div>
										<div class="cztypelb">
											<table border="0" cellpadding="1" cellspacing="1">
												<tr>
													<td>
														<a href="/view/shishicai_cq/ssc_kjhistory.html">历史开奖</a>
													</td>
													<td>
														<a href="/view/shishicai_cq/ssc_luzhufenxizh.html">路珠分析</a>
													</td>
													<td>
														<a href="/view/shishicai_cq/ssc_zhonghefx.html">综合分析</a>
													</td>
													<td>
														<a href="/view/shishicai_cq/ssc_lengrefx.html">冷热分析</a>
													</td>
												</tr>
											</table>
										</div>
									</li>
									<!--

                                    	时间：2016-11-28
                                    	描述：10003 天津时时彩
                                    -->
									<li class="cztypell" id="tjSsc">
										<div class="cztypelt">
											<div class="rowbox1">
												<div class="imgheadl">
													<a href="/view/shishicai_tj/ssc_index.html"><img src="picture/tianjinssc.png" alt="天津时时彩" /></a>
												</div>
												<div class="inforr">
													<span class="nextIssue displaynone"></span>
													<h1>天津时时彩第<span class="preDrawIssue"></span>期开奖</h1>
													<p>十分钟一期</p>
													<p>全天84期</p><br />
													<p>当前<span class="drawCount"></span>期，剩余<span class="sdrawCountnext"></span>期</p>
													<p class="opentyle">开奖中...</p>
													<div class="line linetime cuttime indextime">
														<span class="bgtime hour">...</span>
														<span class="hourtxt">时</span>
														<span class="bgtime minute">...</span>
														<span>分</span>
														<span class="bgtime second">...</span>
														<span>秒</span>
													</div>
												</div>
											</div>
											<div class="rowbox2">
												<ul class="kajianhao">
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
												</ul>
											</div>
											<div class="rowbox3">
												<table border="0" cellpadding="1" cellspacing="1">
													<tr>
														<th width="40" colspan="3">总和</th>
														<th>龙虎</th>
														<th>前三</th>
														<th>中三</th>
														<th>后三</th>
													</tr>
													<tr class="longhu2">
														<td class="sumNum">&nbsp;&nbsp;</td>
														<td class="sumSingleDouble">&nbsp;&nbsp;</td>
														<td class="sumBigSmall">&nbsp;&nbsp;</td>
														<td class="dragonTiger">&nbsp;&nbsp;</td>
														<td class="behindThree">&nbsp;&nbsp;</td>
														<td class="betweenThree">&nbsp;&nbsp;</td>
														<td class="lastThree">&nbsp;&nbsp;</td>
													</tr>
												</table>
											</div>
										</div>
										<div class="cztypelb">
											<table border="0" cellpadding="1" cellspacing="1">
												<tr>
													<td>
														<a href="/view/shishicai_tj/ssc_kjhistory.html">历史开奖</a>
													</td>
													<td>
														<a href="/view/shishicai_tj/ssc_luzhufenxizh.html">路珠分析</a>
													</td>
													<td>
														<a href="/view/shishicai_tj/ssc_zhonghefx.html">综合分析</a>
													</td>
													<td>
														<a href="/view/shishicai_tj/ssc_lengrefx.html">冷热分析</a>
													</td>
												</tr>
											</table>
										</div>
									</li>
									<li class="cztypell" id="xjSsc">
										<div class="cztypelt">
											<div class="rowbox1">
												<div class="imgheadl">
													<a href="/view/shishicai_xj/ssc_index.html"><img src="picture/xinjinagssc.png" alt="新疆时时彩" /></a>
												</div>
												<div class="inforr">
													<span class="nextIssue displaynone"></span>
													<h1>新疆时时彩<span class="preDrawIssue"></span>期开奖</h1>
													<p>十分钟一期</p>
													<p>全天96期</p><br />
													<p>当前<span class="drawCount"></span>期，剩余<span class="sdrawCountnext"></span>期</p>
													<p class="opentyle">开奖中...</p>
													<div class="line linetime cuttime indextime">
														<span class="bgtime hour">...</span>
														<span class="hourtxt">时</span>
														<span class="bgtime minute">...</span>
														<span>分</span>
														<span class="bgtime second">...</span>
														<span>秒</span>
													</div>
												</div>
											</div>
											<div class="rowbox2">
												<ul class="kajianhao">
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
												</ul>
											</div>
											<div class="rowbox3">
												<table border="0" cellpadding="1" cellspacing="1">
													<tr>
														<th width="40" colspan="3">总和</th>
														<th>龙虎</th>
														<th>前三</th>
														<th>中三</th>
														<th>后三</th>
													</tr>
													<tr class="longhu2">
														<td class="sumNum">&nbsp;</td>
														<td class="sumSingleDouble">&nbsp;</td>
														<td class="sumBigSmall">&nbsp;</td>
														<td class="dragonTiger">&nbsp;</td>
														<td class="behindThree">&nbsp;</td>
														<td class="betweenThree">&nbsp;</td>
														<td class="lastThree">&nbsp;</td>
													</tr>
												</table>
											</div>
										</div>
										<div class="cztypelb">
											<table border="0" cellpadding="1" cellspacing="1">
												<tr>
													<td>
														<a href="/view/shishicai_xj/ssc_kjhistory.html">历史开奖</a>
													</td>
													<td>
														<a href="/view/shishicai_xj/ssc_luzhufenxizh.html">路珠分析</a>
													</td>
													<td>
														<a href="/view/shishicai_xj/ssc_zhonghefx.html">综合分析</a>
													</td>
													<td>
														<a href="/view/shishicai_xj/ssc_lengrefx.html">冷热分析</a>
													</td>
												</tr>
											</table>
										</div>
									</li>
									<!--

                                    	时间：2016-11-28
                                    	描述：广东快乐十分
                                    -->
									<li class="cztypell" id="gdklsf">
										<div class="cztypelt">
											<div class="rowbox1">
												<div class="imgheadl">
													<a href="/view/klsf/klsf_index.html"><img src="picture/guangdongklsf.png" alt="广东快乐十分" /></a>
												</div>
												<div class="inforr">
													<span class="nextIssue displaynone"></span>
													<h1>广东快乐十分<span class="preDrawIssue"></span>期开奖</h1>
													<p>每10分钟一期,全天84期</p><br/>
													<p>当前<span class="drawCount"></span>期，剩余<span class="sdrawCountnext"></span>期</p>
													<p class="opentyle">开奖中...</p>
													<div class="line linetime cuttime indextime">
														<span class="bgtime hour">...</span>
														<span class="hourtxt">时</span>
														<span class="bgtime minute">...</span>
														<span>分</span>
														<span class="bgtime second">...</span>
														<span>秒</span>
													</div>
												</div>
											</div>
											<div class="rowbox2">
												<ul class="kajianhao">
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
												</ul>
											</div>
											<div class="rowbox3">
												<table border="0" cellpadding="1" cellspacing="1">
													<tr>
														<th width="40" colspan="4">总和</th>
														<th colspan="4">1-4龙虎</th>
													</tr>
													<tr class="longhu2">
														<td class="sumNum">&nbsp;</td>
														<td class="sumSingleDouble">&nbsp;</td>
														<td class="sumBigSmall">&nbsp;</td>
														<td class="lastBigSmall">&nbsp;</td>

														<td class="firstDragonTiger">&nbsp;</td>
														<td class="secondDragonTiger">&nbsp;</td>
														<td class="thirdDragonTiger">&nbsp;</td>
														<td class="fourthDragonTiger">&nbsp;</td>
													</tr>
												</table>
											</div>
										</div>
										<div class="cztypelb">
											<table border="0" cellpadding="1" cellspacing="1">
												<tr>
													<td>
														<a href="/view/klsf/klsf_kjhistory.html">历史开奖</a>
													</td>
													<td>
														<a href="/view/klsf/klsf_zonghelzfx.html">路珠分析</a>
													</td>
													<td>
														<a href="/view/klsf/klsf_danshuangdxls.html">单双大小历史</a>
													</td>
													<td>
														<a href="/view/klsf/klsf_jibenzs.html">基本走势</a>
													</td>
												</tr>
											</table>
										</div>
									</li>
									<li class="cztypell" id="shiyix5_sd">
										<div class="cztypelt">
											<div class="rowbox1">
												<div class="imgheadl">
													<a href="/view/shiyix5_sd/index.html"><img src="picture/shandong11x5.png" alt="十一运夺金" /></a>
												</div>
												<div class="inforr">
													<span class="nextIssue displaynone"></span>
													<h1>十一运夺金第<span class="preDrawIssue"></span>期开奖</h1>
													<p>每10分钟一期,全天87期</p><br/>
													<p>当前<span class="drawCount"></span>期，剩余<span class="sdrawCountnext"></span>期</p>
													<p class="opentyle">开奖中...</p>
													<div class="line linetime cuttime indextime">
														<span class="bgtime hour">...</span>
														<span class="hourtxt">时</span>
														<span class="bgtime minute">...</span>
														<span>分</span>
														<span class="bgtime second">...</span>
														<span>秒</span>
													</div>
												</div>
											</div>
											<div class="rowbox2">
												<ul class="kajianhao">
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
												</ul>
											</div>
											<div class="rowbox3">
												<table border="0" cellpadding="1" cellspacing="1">
													<tr>
														<th width="40" colspan="3">总和</th>
														<th>前三</th>
														<th>中三</th>
														<th>后三</th>
													</tr>
													<tr class="longhu2">
														<td class="sumNum">&nbsp;</td>
														<td class="sumBigSmall">&nbsp;</td>
														<td class="sumSingleDouble">&nbsp;</td>
														<td class="behindThree">&nbsp;</td>
														<td class="betweenThree">&nbsp;</td>
														<td class="lastThree">&nbsp;</td>
													</tr>
												</table>
											</div>
										</div>
										<div class="cztypelb">
											<table border="0" cellpadding="1" cellspacing="1">
												<tr>
													<td>
														<a href="/view/shiyix5_sd/kjhistory.html">历史开奖</a>
													</td>
													<td>
														<a href="/view/shiyix5_sd/zonghelzfx.html">路珠分析</a>
													</td>
													<td>
														<a href="/view/shiyix5_sd/danshuangdxls.html">单双大小历史</a>
													</td>
													<td>
														<a href="/view/shiyix5_sd/jibenzs.html">基本走势</a>
													</td>
												</tr>
											</table>
										</div>
									</li>
									<li class="cztypell" id="shiyix5_gd">
										<div class="cztypelt">
											<div class="rowbox1">
												<div class="imgheadl">
													<a href="/view/shiyix5_gd/index.html"><img src="picture/gaungdong11x5.png" alt="广东11选5" /></a>
												</div>
												<div class="inforr">
													<span class="nextIssue displaynone"></span>
													<h1>广东11选5第<span class="preDrawIssue"></span>期开奖</h1>
													<p>每10分钟一期,全天84期</p><br/>
													<p>当前<span class="drawCount"></span>期，剩余<span class="sdrawCountnext"></span>期</p>
													<p class="opentyle">开奖中...</p>
													<div class="line linetime cuttime indextime">
														<span class="bgtime hour">...</span>
														<span class="hourtxt">时</span>
														<span class="bgtime minute">...</span>
														<span>分</span>
														<span class="bgtime second">...</span>
														<span>秒</span>
													</div>
												</div>
											</div>
											<div class="rowbox2">
												<ul class="kajianhao">
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
													<li class="numblueHead"></li>
												</ul>
											</div>
											<div class="rowbox3">
												<table border="0" cellpadding="1" cellspacing="1">
													<tr>
														<th width="40" colspan="3">总和</th>
														<th>前三</th>
														<th>中三</th>
														<th>后三</th>
													</tr>
													<tr class="longhu2">
														<td class="sumNum">&nbsp;</td>
														<td class="sumBigSmall">&nbsp;</td>
														<td class="sumSingleDouble">&nbsp;</td>

														<td class="behindThree">&nbsp;</td>
														<td class="betweenThree">&nbsp;</td>
														<td class="lastThree">&nbsp;</td>
													</tr>
												</table>
											</div>
										</div>
										<div class="cztypelb">
											<table border="0" cellpadding="1" cellspacing="1">
												<tr>
													<td>
														<a href="/view/shiyix5_gd/kjhistory.html">历史开奖</a>
													</td>
													<td>
														<a href="/view/shiyix5_gd/zonghelzfx.html">路珠分析</a>
													</td>
													<td>
														<a href="/view/shiyix5_gd/danshuangdxls.html">单双大小历史</a>
													</td>
													<td>
														<a href="/view/shiyix5_gd/jibenzs.html">基本走势</a>
													</td>
												</tr>
											</table>
										</div>
									</li>
									<li class="cztypell" id="kuai3">
										<div class="cztypelt">
											<div class="rowbox1">
												<div class="imgheadl">
													<a href="/view/kuai3/kuai3_index.html"><img src="picture/jskuai3.png" alt="江苏快3" /></a>
												</div>
												<div class="inforr">
													<span class="nextIssue displaynone"></span>
													<h1>江苏快3第<span class="preDrawIssue"></span>期开奖</h1>
													<p>每10分钟一期，全天82期</p><br/>
													<p>当前<span class="drawCount"></span>期，剩余<span class="sdrawCount"></span>期</p>
													<p class="opentyle">开奖中...</p>
													<div class="line linetime cuttime indextime">
														<span class="bgtime hour">...</span>
														<span class="hourtxt">时</span>
														<span class="bgtime minute">...</span>
														<span>分</span>
														<span class="bgtime second">...</span>
														<span>秒</span>
													</div>
												</div>
											</div>
											<div class="rowbox2">
												<ul class="kajianhao">
													<li class="num1"></li>
													<li class="num2"></li>
													<li class="num4"></li>
												</ul>
											</div>
											<div class="rowbox3">
												<table border="0" cellpadding="1" cellspacing="1">
													<tr>
														<th colspan="3">总和</th>
														<th colspan="3">鱼虾蟹</th>
													</tr>
													<tr>
														<td class="sumNum">&nbsp;</td>
														<td class="sumSingleDouble">&nbsp;</td>
														<td class="sumBigSmall">&nbsp;</td>
														<td class="firstSeafood">&nbsp;</td>
														<td class="secondSeafood">&nbsp;</td>
														<td class="thirdSeafood">&nbsp;</td>
													</tr>
												</table>
											</div>
										</div>
										<div class="cztypelb">
											<table border="0" cellpadding="1" cellspacing="1">
												<tr>
													<td>
														<a href="/view/kuai3/kuai3index_history.html">历史开奖</a>
													</td>
													<td>
														<a href="/view/kuai3/kuai3_meiricltj.html">每日长龙统计</a>
													</td>
													<td>
														<a href="/view/kuai3/kuai3_zonghelz.html">总和路珠</a>
													</td>
													<td>
														<a href="/view/kuai3/kuai3_haomalz.html">号码路珠</a>
													</td>
												</tr>
											</table>
										</div>
									</li>
								</ul>
							</div>

						</div>
					</div>
				</div>
			</div>
			<div class="footer1">
	<ul>
		<li class="lileft">
			<div style="width: 100px;height: 60px">
			<img src="/style/images/logo.gif"  width="100" height="60">
			</div>
			<p>最专业的彩票开奖网站</p>
			<p>数据分析最全面的开奖数据平台 </p>
		</li>
		<li class="about_li">
			<div><span class="about"></span><span class="">关于我们</span></div>
			<p class="p1">
				<a href="/view/about_us/about_us.html">关于我们</a>
			</p>
			<p>
				<a href="/view/about_us/cus_serv.html">客服中心</a>
			</p>
			<p>
				<a href="/view/about_us/coop_com.html">商务合作</a>
			</p>
			<p>
				<a href="/view/about_us/disclaimer.html">免责声明</a>
			</p>
		</li>
		<li class="about_li">
			<div><span class="rewards"></span><span class="">中奖神器</span></div>
			<p class="p1">
				<a href="/view/PK10/pk10kai.html">开奖号码</a>
			</p>
			<p>
				<a href="/view/zoushitb/zoushitb.html">走势图表</a>
			</p>
			<p>
				<a href="/view/wanfaguize/PK10_wanfaguize.html">玩法规则</a>
			</p>
			<!--<p>
							<a href="javascript:void(0)">遗漏数据</a>
						</p>-->
		</li>
		<li class="about_li">
			<div><span class="call"></span><span class="">免费调用</span></div>
			<!--<p class="p1">
							<a href="javascript:void(0)">彩票开奖联盟</a>
						</p>-->
			<p class="p1">
				<a href="http://168kai.com/" class="zzwzdh" target="_blank" rel="nofollow">自助网址导航</a>
			</p>
			<p>
				<a  class="kaijdy" href="http://kj.kai861.com/" target="_blank" rel="nofollow">开奖调用</a>
			</p>
		</li>
		<li>
			<!--<div><img src="picture/gongzh.png" alt="" style="width: 100%;"/></div>
			<p>扫一扫，关注168公众号</p>-->
		</li>
	</ul>
</div>

<div class="footer3">
	<div class="footer3c">
		Copyright <span id="localyears"></span> www.168kai.com All rights reserved 彩票开奖网 版权所有 粤ICP备09298233号
		<a style="display: inline;margin-left: 5px;" href="http://www.cnzz.com/stat/website.php?web_id=1261504940" target="_blank" title="站长统计">站长统计</a>
		<script type="text/javascript" src="/js/js/tools.js" ></script>
		<script type="text/javascript">
			$("#localyears").text(new Date().getFullYear());
			toolBoxs.setK();//为开奖调用增加域名
			var logoLocation = window.location.href.indexOf("zsh/index"); //如果是站长推荐页面，就修改底部的彩种logo图片
			if(logoLocation !== -1){
				$(".lileft").find("img").attr("src","../../images/images/zshImage/logoBottom.png?v=201889947");
			}
		</script>
	</div>
</div>
		</div>
		<div class="fixedgoBack">
			<div class="fix1200">
				<ul>
					<li>
						<a class="kefuicon" target="_blank" href="http://crm2.qq.com/page/portalpage/wpa.php?lang=&uin=800057725&cref=http://www.168kai.cc&ref=&pt=168%E5%BC%80%E5%A5%96%E7%BD%91&f=1&ty=1&ap=&as="></a>
					</li>
					<li>
						<!--用户反馈模态框-->
						<span class="fankuicon fankuicon_a" data-toggle="modal" data-target="#myModal">
							<span class="fankuicons"></span>
						</span>
					</li>
					<li>
						<a class="topicon" id="gotop" href="javascript:"></a>
					</li>
				</ul>
			</div>
		</div>

		<!--模态框-->
		<!-- 模态框（Modal） -->
		<div id="user_adv">
			<!--模态框-->
			<!-- 模态框（Modal） -->
			<div class="modal fade" style="z-index: 999;" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				<div class="modal-dialog" style="margin: 30px auto;margin-top: 130px;">
					<div class="modal-content">
						<div class="modal-header">
							<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><img src="picture/close.png" alt="" /></button>
							<h4 class="modal-title" id="myModalLabel">在线反馈</h4>
						</div>
						<div class="modal-body">
							<div class="info" id="info1" style="display: block;">
								<div class="div_top">欢迎对开奖网提出宝贵的建议或意见</div>
								<div class="div_middle">
									<div class="middle_l">
										<div class="l_div">称呼</div>
										<div class="r_div"><input type="" placeholder="" class="input_l" id="nickName" /></div>
									</div>
									<div class="middle_r">
										<select name="" class="middle_select" id="linkType">
											<option value="1">电话</option>
											<option value="2">邮箱</option>
											<option value="3">QQ</option>
											<option value="4">微信</option>
										</select>
										<input type="text" class="input_r" id="linkNumber" />
									</div>
								</div>
								<div class="div_down">
									<textarea name="" rows="" cols="" id="advice" placeholder="请写下您宝贵的意见..."></textarea>
								</div>
								<div>
									<button type="button" class="btn btn_submit" id="btn_submit">提交</button>
									<button type="button" class="btn btn_submiting" id="btn_submiting">正在提交...</button>
								</div>
							</div>
							<!--info1-->
							<div class="info" id="info2" style="display: none;">
								<div class="success_div">
									<div class="pic"><img src="picture/tishi.png" alt="" /></div>
									<div class="success_h">
										<h1>提交成功</h1>
										<p>感谢您反馈，我们将尽快做出相应的调整！</p>
									</div>
								</div>
							</div>
							<!--info2-->
							<div class="info" id="info3" style="display: none;">
								<div class="success_div">
									<div class="pic"><img src="picture/fankuidh.gif" alt="" /></div>
									<div class="success_h">
										<h1>您已提交</h1>
										<p>请您稍适休息，喝杯咖啡，谢谢您的支持！</p>
									</div>
								</div>
							</div>
							<!--info2-->
						</div>
					</div>
				</div>
			</div>
		</div>

	</body>
	<script type="text/javascript" src="/js/js/ga.js"></script>
	<script type="text/javascript" src="/js/js/jquery.flexslider-min.js"></script>
	<script type="text/javascript" src="/js/js/pk10_index.js"></script>
	<script type="text/javascript" src="/js/js/animate.js"></script>
	<script type="text/javascript" src="/js/js/index.js"></script>
	<script src="/js/js/idangerous.swiper.min.js"></script>
	<script>
		indexObj.loadBanner();
	</script>
	<div id="videobox">
		<div class="content">
			<div class="head">
				<i id="vtitle"></i>
				<div class="btn">
					<ul>
						<li class="closevideo"><i class="iconfont"></i></li>
						<!--<li class="small">小屏</li>-->
						<li class="big">中屏</li>
					</ul>
				</div>
			</div>
			<div class="animate" id="videoIframe">
				<iframe style="height:100%;width:100%;border: none;" scrolling="no" src=""></iframe>
			</div>
		</div>
	</div>

</html>