 ...
 //   console.log(webListOP);
    if (list.length > 0) {
        ...
        $('#web_listOP_ul').html(html);
				...
				<!--  内容开始 -->
					 <div id="lot-wrap">
					 <input type="hidden" id="callFun" value="getBallStat"/>
						<div id="ball_stat_list-html" style="float: left;"></div> 
						<script id="ball_stat_list-tmpl" type="text/template">
							<table class="lot-table" id="ballstatTable">
							    <caption>
							    </caption>
							    <tr class="head">
							        <td width="160">日期</td>
							        <td width="85px">冠　军</td>
							        <td width="85px">亚　军</td>
							        <td width="85px">第三名</td>
							        <td width="85px">第四名</td>
							        <td width="85px">第五名</td>
							        <td width="85px">第六名</td>
							        <td width="85px">第七名</td>
							        <td width="85px">第八名</td>
							        <td width="85px">第九名</td>
							        <td width="85px">第十名</td>
							    </tr>
							    ...
							    {{ 
								    ...
								    <tr class="{{=(i+1)%2==0?'even':'odd'}}"> 
								     <td>{{=new Date(Date.parse(ld.replace(/-/g,"/"))).format("yyyy-MM-dd")}} </td> 
								     <td colspan="20"> 
								      <table class="ball"> 
								       <tbody>
								        <tr> 
								         <td class="bg1">{{=getVal(it,ld,1,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,1,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,1,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,1,5,2)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,2,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,2,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,2,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,2,5,2)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,3,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,3,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,3,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,3,5,2)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,4,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,4,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,4,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,4,5,2)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,5,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,5,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,5,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,5,5,2)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,6,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,6,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,6,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,6,5,2)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,7,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,7,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,7,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,7,5,2)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,8,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,8,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,8,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,8,5,2)}} </td> 

								         <td class="bg1">{{=getVal(it,ld,9,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,9,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,9,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,9,5,2)}} </td> 

								         <td class="bg1">{{=getVal(it,ld,10,6,2)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,10,6,1)}} </td> 
								         <td class="bg1">{{=getVal(it,ld,10,5,1)}} </td> 
								         <td class="bg2">{{=getVal(it,ld,10,5,2)}} </td> 
        			...
        			<script type="text/javascript">
						function getBallStat(){
							$.get("pk10/getBallStat.do",function(r){
								if (r) {
